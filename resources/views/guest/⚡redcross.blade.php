    ...
    <div wire:transition>
      ...
      <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
        ...
        <div class="mt-4 space-y-3" wire:transition>
          ...
          <flux:select wire:model="personal.course_id" label="Course *">
            ...
            <flux:select.option wire:key="redcross-representative-course-{{ $courseOption['id'] }}"
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-gray-100 pt-5">
          ...
          @if (! $is_representative)
          <flux:select wire:model="personal.course_id" label="Course *" class="sm:col-span-2">
            <flux:select.option value="">Select course...</flux:select.option>
            @foreach ($courseOptions as $courseOption)
            <flux:select.option wire:key="redcross-course-{{ $courseOption['id'] }}"
              value="{{ $courseOption['id'] }}">
              {{ $courseOption['name'] }} ({{ $courseOption['department'] ?: 'No Department' }})
            </flux:select.option>
            @endforeach
