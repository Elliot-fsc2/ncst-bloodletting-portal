...
new class extends Component {
  ...
  protected function rulesForStep(int $step): array
  {
    return match ($step) {
      1 => [
        ...
        'personal.course_id' => 'required|exists:courses,id',
  ...
  protected function messagesForStep(int $step): array
  {
    return match ($step) {
      1 => [
        ...
        'personal.course_id.required' => 'Course is required.',
        'personal.course_id.exists' => 'Please select a valid course.',
  ...
  protected function syncSelectedCourse(): bool
  {
    $selectedCourse = Course::query()->with('department:id,name')->find((int) ($this->personal['course_id'] ?? 0));

    $this->personal['course'] = $selectedCourse?->name ?? '';
    $this->personal['department'] = $selectedCourse?->department?->name ?? '';
  ...
  public function submit(): void
  {
    ...
    if (! $this->syncSelectedCourse()) {
      $this->addError('personal.course_id', 'Please select a valid course.');
      $this->step = 1;
      $this->js('window.scrollTo({top: 0, behavior: "smooth"})');
      return;
    }

    $personalData = $this->personal;
    unset($personalData['course_id']);
  ...
  <div class="mb-6">
    <div class="flex items-center justify-between relative max-w-lg mx-auto">
      ...
      <div class="flex flex-col items-center z-10 gap-1.5 px-2">
        <div @class([ 'flex items-center justify-center w-10 h-10 rounded-full text-sm font-bold border-2 transition-all duration-300' , 'bg-red-600 border-red-600 text-white shadow-md shadow-red-200'=>
          $step === $n,
        ...
        <span @class([ 'text-[0.65rem] font-medium hidden sm:block transition-colors duration-300' , 'text-red-600'=> $step >= $n,
  ...
  <flux:card class="shadow-sm">
    ...
    <div wire:transition>
      ...
      <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
        ...
        <div class="mt-4 space-y-3" wire:transition>
          ...
          <flux:select wire:model="personal.course_id" label="Course *">
            ...
            <flux:select.option wire:key="pgh-representative-course-{{ $courseOption['id'] }}"
      ...
      <div class="space-y-5">
        ...
        {{-- Contact Details --}}
        <div>
          <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Contact Details</p>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <flux:input wire:model="personal.email" label="Email Address *" type="email" placeholder="e.g. juan@example.com" />
            <flux:input wire:model="personal.telephone" label="Mobile/Telephone No." placeholder="e.g. 09123456789" />
        ...
        <flux:select wire:model="personal.course_id" label="Course *">
          <flux:select.option value="">Select course...</flux:select.option>
          @foreach ($courseOptions as $courseOption)
          <flux:select.option wire:key="pgh-course-{{ $courseOption['id'] }}"
            value="{{ $courseOption['id'] }}">
            {{ $courseOption['name'] }} ({{ $courseOption['department'] ?: 'No Department' }})
          </flux:select.option>
          @endforeach
