...
new class extends Component {
  ...
  protected function messagesForStep(int $step): array
  {
    ...
    $msgs['personal.course_id.exists'] = 'Please select a valid course.';
  ...
  public function nextStep(): void
  {
    ...
    if ($this->step === 1 && Form::where('donor_email', $this->personal['email'] ?? '')->exists()) {
    ...
    <div wire:transition>
      ...
      <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
        ...
        <div class="mt-4 space-y-3" wire:transition>
          ...
          <flux:select wire:model="personal.course_id" label="Course *">
            ...
            <flux:select.option wire:key="vmmc-representative-course-{{ $courseOption['id'] }}"
      ...
      <div class="space-y-5">
        ...
        <flux:select wire:model="personal.course_id" label="Course *">
          <flux:select.option value="">Select course...</flux:select.option>
          @foreach ($courseOptions as $courseOption)
          <flux:select.option wire:key="vmmc-course-{{ $courseOption['id'] }}"
            value="{{ $courseOption['id'] }}">
            {{ $courseOption['name'] }} ({{ $courseOption['department'] ?: 'No Department' }})
          </flux:select.option>
          @endforeach
...
<div class="flex items-center justify-between gap-3 mt-8 pt-5 border-t border-gray-100">
  ...
  <div class="flex items-center gap-3">
    ...
    @if ($step < 2)
      ...
      @else
      <flux:button wire:click="submit" variant="primary" x-bind:disabled="! $wire.consent" icon="check"
