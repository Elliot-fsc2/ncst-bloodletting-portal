    ...
    <flux:card class="shadow-sm">
        ...
        @if($step === 1)
            <div wire:transition>
                ...
                <div class="space-y-5">
                    ...
                    <flux:input wire:model="personal.occupation" label="Occupation *" placeholder="e.g. Student, Engineer..." />

                    <div class="pt-1">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Address</p>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <flux:input wire:model="personal.house_no" label="House No / Street" placeholder="123 Rizal St." />
        ...
        @if($step === 2)
            <div wire:transition>
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-red-100">
                    ...
                    <div>
                        <flux:heading class="text-base! font-semibold! text-gray-800!">General Medical History</flux:heading>
            ...
            <div wire:transition>
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-xl border border-amber-100 bg-amber-50/60 p-4">
                        <p class="text-xs font-bold text-amber-700 uppercase tracking-wider mb-3">Section A — Past 18 Months</p>
                    ...
                    <div class="rounded-xl border border-gray-100 bg-gray-50/60 p-4 space-y-5">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Section B — Past 12 Months</p>
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-[0.7rem] text-gray-500 italic leading-relaxed pl-1">
                                Malaria, hepatitis, jaundice, syphilis, chicken pox, shingles, cold sores, serious accident, cancer, blood disease (e.g. leukemia), recent/severe respiratory disease, cardiovascular disease, kidney disease, diabetes, asthma, epilepsy, tuberculosis.
                            </p>
                        </div>
                        ...
                        {{-- B13 --}}
                        <flux:radio.group wire:model="section_b.jail"
                            label="B13. Been in jail or prison?">
                            <flux:radio value="yes" label="Yes" />
                            <flux:radio value="no" label="No" />
                        </flux:radio.group>
                    </div>
        ...
        @if($step === 4)
            <div wire:transition>
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-red-100">
                    ...
                    <div>
                        <flux:heading class="text-base! font-semibold! text-gray-800!">Recent Status & COVID-19</flux:heading>
                ...
                <div class="space-y-4">
                    {{-- Section C --}}
                    <div class="rounded-xl border border-gray-100 bg-gray-50/60 p-4 space-y-5">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Section C — Past 4 Weeks / 24 Hours</p>
                    ...
                    <div class="rounded-xl border border-blue-100 bg-blue-50/50 p-4 space-y-5">
                        <p class="text-xs font-bold text-blue-700 uppercase tracking-wider">Section D — COVID-19 (Past 28 Days)</p>
                            ...
                            @if($section_d['travel_intl'] === 'yes')
                                <div wire:transition>
                                    <flux:input wire:model="section_d.travel_countries"
                                        label="Indicate the country/ies visited"
                            ...
                            @if($section_d['vaccine_received'] === 'yes')
                                <div wire:transition>
                                    <flux:input wire:model="section_d.vaccine_details"
                                        label="Indicate the type of vaccine"
            ...
            <div wire:transition>
                ...
                <div class="space-y-5 text-sm">
                    ...
                    <div class="rounded-xl border border-gray-200 overflow-hidden">
                        <div class="flex items-center gap-2 bg-red-50 px-4 py-2.5 border-b border-red-100">
                            <span>🩺</span>
                            <p class="text-xs font-bold text-red-700 uppercase tracking-wider">General Medical History</p>
                        </div>
                        <div class="divide-y divide-gray-100">
                            @foreach([
                                ['label' => 'Donated before?',          'val' => $history['donated_before']],
                                ['label' => 'Last donation info',        'val' => $history['last_donation_info'] ?: '—', 'skip' => $history['donated_before'] !== 'yes'],
                                ['label' => 'Used different name?',      'val' => $history['used_different_name']],
                                ['label' => 'Deferred before?',          'val' => $history['deferred_before']],
                            ] as $row)
                                @if(empty($row['skip']))
                    ...
                    <div class="rounded-xl border border-amber-200 overflow-hidden">
                        <div class="flex items-center gap-2 bg-amber-50 px-4 py-2.5 border-b border-amber-100">
                            <span>📋</span>
                            <p class="text-xs font-bold text-amber-700 uppercase tracking-wider">Section A — Past 18 Months</p>
                        </div>
                        <div class="divide-y divide-gray-100">
                            <div class="flex items-center justify-between px-4 py-2.5 bg-white">
                                <p class="text-xs text-gray-600 pr-4">High blood pressure, night sweats, unexplained fevers/weight loss, persistent diarrhea, or enlarged lymph node?</p>
                    ...
                    <div class="rounded-xl border border-gray-200 overflow-hidden">
                        <div class="flex items-center gap-2 bg-red-50 px-4 py-2.5 border-b border-red-100">
                            <span>📋</span>
                            <p class="text-xs font-bold text-red-700 uppercase tracking-wider">Section B — Past 12 Months</p>
                        </div>
                        <div class="divide-y divide-gray-100">
                            @foreach([
                    ...
                    <div class="rounded-xl border border-gray-200 overflow-hidden">
                        <div class="flex items-center gap-2 bg-red-50 px-4 py-2.5 border-b border-red-100">
                            <span>🕐</span>
                            <p class="text-xs font-bold text-red-700 uppercase tracking-wider">Section C — Past 4 Weeks / 24 Hours</p>
                        </div>
                        <div class="divide-y divide-gray-100">
                            @foreach([
                    ...
                    @if($personal['sex'] === 'Female')
                        <div class="rounded-xl border border-pink-200 overflow-hidden">
                            <div class="flex items-center gap-2 bg-pink-50 px-4 py-2.5 border-b border-pink-100">
                                <span>🌸</span>
                                <p class="text-xs font-bold text-pink-700 uppercase tracking-wider">Section E — Female Donors</p>
                            </div>
                            <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white">
                                <flux:input wire:model="section_e.delivery" label="E1. Last Delivery Date" type="date" />
                                <flux:input wire:model="section_e.menstruation" label="E2. Last Menstruation Date" type="date" />
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="rounded-xl border border-red-200 bg-red-50/40 p-4">
                        <div class="flex gap-3 mb-4">
                            <svg class="w-5 h-5 text-red-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            <p class="text-xs text-gray-600 italic leading-relaxed">
                                "I voluntarily give my consent for the collection, use, and processing of my personal data to <strong class="not-italic font-semibold text-red-700">OPERATION LIFELINE – VMMC</strong>. I declare that I have truthfully answered all of the above questions."
                            </p>
                        </div>
                        <flux:checkbox wire:model.live="consent" label="I have read and understand the consent terms." />
                        @error('consent') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
        ...
        <div class="flex items-center justify-between gap-3 mt-8 pt-5 border-t border-gray-100">
            ...
            <div class="flex items-center gap-3">
                ...
                @if($step < 5)
                    <flux:button wire:click="nextStep" variant="primary" icon:trailing="chevron-right">Next</flux:button>
                @else
                    <flux:button wire:click="submit" variant="primary" :disabled="!$consent" icon="check">Submit</flux:button>
                @endif
            </div>
        </div>
