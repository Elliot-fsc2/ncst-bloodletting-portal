...
class SendRedCrossDonorPdfEmail implements ShouldQueue
{
  use Queueable;
  ...
  {
    $pdfSubpath = 'redcross/' . $this->filename;
    $pdfAbsolutePath = Storage::disk('local')->path('private/pdfs/' . $pdfSubpath);

    File::ensureDirectoryExists(Storage::disk('local')->path('private/pdfs/redcross'));

    Pdf::view('pdf.redcross-pdf', ['data' => $this->pdfData['personal']])
      ->margins(1, 10, 1, 10)
      ->format('a4')
